...
async fn foobar() -> gazette::Result<()> {
    ...
    let router =
        gazette::journal::Router::new("http://localhost:8080", gazette::Auth::new(None)?, "local")?;
    ...
    let stream = client.read_json_lines(rr);
    tokio::pin!(stream);
